  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        const womensProducts = allProducts.filter((product) => product.gender === 'Female');
        setProducts(womensProducts);
        setDisplayedProducts(womensProducts);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  return (
    <>
      {products.length === 0 ? (
        ...
        <>
          <h1 className="products-header">Women's Products</h1>
